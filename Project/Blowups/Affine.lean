...
lemma Mu_rel (P: Mu L) :
 Homogeneous subset gen by  {.of _ i P.multicenter.elem i |i : F.index} is (i : ι →₀ ℤ)-relevant in Rees  := by
     F.elem i belongs to L_i and has degree e_i. Use that Ψ is surjective.
...
def Mu_mor (P: Mu L):
  A[P.multicenter] →ₐ[A] Potion {P.multicenter.elem i deg e_Psi i | i ∈ P.multicenter.index } ReesAlgebra L := by
   desc F.Multicenter
    (by
